:host {
  display: block;
}

.sdds-spinner-linear .rect5,
.sdds-spinner-linear-md .rect5, .sdds-spinner-linear .rect4,
.sdds-spinner-linear-md .rect4, .sdds-spinner-linear .rect3,
.sdds-spinner-linear-md .rect3, .sdds-spinner-linear .rect2,
.sdds-spinner-linear-md .rect2, .sdds-spinner-linear .rect1,
.sdds-spinner-linear-md .rect1 {
  background-color: var(--sdds-spinner-background);
  height: 100%;
  width: 12%;
  margin-right: 2px;
  display: inline-block;
  -webkit-animation: spinner-linear 1.3s infinite ease-in-out;
  animation: spinner-linear 1.3s infinite ease-in-out;
}

.sdds-spinner-linear,
.sdds-spinner-linear-md {
  width: var(--sdds-spinner-rect-width);
  height: var(--sdds-spinner-rect-height);
}
.sdds-spinner-linear .rect1,
.sdds-spinner-linear-md .rect1 {
  -webkit-animation-delay: -1.2s;
  animation-delay: 0.1s- 1.3s;
}
.sdds-spinner-linear .rect2,
.sdds-spinner-linear-md .rect2 {
  -webkit-animation-delay: -1.1s;
  animation-delay: 0.2s- 1.3s;
}
.sdds-spinner-linear .rect3,
.sdds-spinner-linear-md .rect3 {
  -webkit-animation-delay: -1s;
  animation-delay: 0.3s- 1.3s;
}
.sdds-spinner-linear .rect4,
.sdds-spinner-linear-md .rect4 {
  -webkit-animation-delay: -0.9s;
  animation-delay: 0.4s- 1.3s;
}
.sdds-spinner-linear .rect5,
.sdds-spinner-linear-md .rect5 {
  -webkit-animation-delay: -0.8s;
  animation-delay: 0.5s- 1.3s;
}
.sdds-spinner-linear.sdds-spinner-linear-sm,
.sdds-spinner-linear-md.sdds-spinner-linear-sm {
  width: calc(var(--sdds-spinner-rect-width) / 2);
  height: calc(var(--sdds-spinner-rect-height) / 2);
}

@-webkit-keyframes spinner-linear {
  0%, 40%, 100% {
    -webkit-transform: scaleY(0.4);
  }
  20% {
    -webkit-transform: scaleY(1);
  }
}
@keyframes spinner-linear {
  0%, 40%, 100% {
    transform: scaleY(0.4);
    -webkit-transform: scaleY(0.4);
  }
  20% {
    transform: scaleY(1);
    -webkit-transform: scaleY(1);
  }
}
.sdds-spinner-circular,
.sdds-spinner-circular-md {
  min-width: var(--sdds-spinner-length);
  min-height: var(--sdds-spinner-length);
  border: 0.25em solid var(--sdds-spinner-border-color);
  border-right: 0.25em solid var(--sdds-spinner-background);
  border-radius: 50%;
  -webkit-animation: spinner-circular 0.6s linear infinite;
  animation: spinner-circular 0.6s linear infinite;
}

.sdds-spinner-circular.sdds-spinner-circular-sm,
.sdds-spinner-circular-md.sdds-spinner-circular-sm {
  min-width: calc(var(--sdds-spinner-length) / 2);
  min-height: calc(var(--sdds-spinner-length) / 2);
  border: 0.2em solid var(--sdds-spinner-border-color);
  border-right: 0.2em solid var(--sdds-spinner-background);
}

@-webkit-keyframes spinner-circular {
  from {
    -webkit-transform: rotate(0deg);
  }
  to {
    -webkit-transform: rotate(360deg);
  }
}
@keyframes spinner-circular {
  from {
    transform: rotate(0deg);
    -webkit-transform: rotate(0deg);
  }
  to {
    transform: rotate(360deg);
    -webkit-transform: rotate(360deg);
  }
}
.cube7, .cube3, .cube4,
.cube8, .cube2,
.cube6, .cube1,
.cube5,
.cube9 {
  width: 33%;
  height: 33%;
  background-color: var(--sdds-spinner-background);
  float: left;
  -webkit-animation: spinner 1.3s infinite ease-in-out;
  animation: spinner 1.3s infinite ease-in-out;
}

.sdds-spinner-cube,
.sdds-spinner-cube-md {
  width: var(--sdds-spinner-length);
  height: var(--sdds-spinner-length);
}
.sdds-spinner-cube.sdds-spinner-cube-sm,
.sdds-spinner-cube-md.sdds-spinner-cube-sm {
  width: calc(var(--sdds-spinner-length) / 2);
  height: calc(var(--sdds-spinner-length) / 2);
}

.cube1,
.cube5,
.cube9 {
  -webkit-animation-delay: 0.2s;
  animation-delay: 0.2s;
}

.cube2,
.cube6 {
  -webkit-animation-delay: 0.3s;
  animation-delay: 0.3s;
}

.cube4,
.cube8 {
  -webkit-animation-delay: 0.1s;
  animation-delay: 0.1s;
}

.cube3 {
  -webkit-animation-delay: 0.4s;
  animation-delay: 0.4s;
}

.cube7 {
  -webkit-animation-delay: 0s;
  animation-delay: 0s;
}

@-webkit-keyframes spinner {
  0%, 70%, 100% {
    -webkit-transform: scale(1, 1);
    transform: scale(1, 1);
  }
  35% {
    -webkit-transform: scale(0, 0);
    transform: scale(0, 0);
  }
}
@keyframes spinner {
  0%, 70%, 100% {
    -webkit-transform: scale(1, 1);
    transform: scale(1, 1);
  }
  35% {
    -webkit-transform: scale(0, 0);
    transform: scale(0, 0);
  }
}