:host {
  display: block;
}

.container {
  display: flex;
  flex-direction: row;
  flex-wrap: nowrap;
  justify-content: space-between;
  height: 20px;
  align-items: center;
  position: relative;
}
.container--continuous {
  margin-top: 100px;
}

div .range-slider {
  width: 100%;
  height: var(--sdds-slider-thumb-diameter);
  float: none;
  text-align: center;
  margin-left: 10px;
  position: relative;
}
div .range-slider [type=range] {
  position: absolute;
}

[type=range] {
  --range: calc(var(--max) - var(--min));
  --ratio: calc((var(--val) - var(--min)) / var(--range));
  --minvalpx: calc(100% * (((var(--minval) - var(--min)) / var(--range))));
  --maxvalpx: calc(100% * (((var(--maxval) - var(--min)) / var(--range))));
  --minrange: calc((var(--maxvalpx) - var(--minvalpx)));
  --sx: calc(var(--ratio) * 100%);
  margin: 0;
  padding: 0;
  width: 100%;
  height: var(--sdds-slider-thumb-diameter);
  background: transparent;
  border: 1px;
  display: block;
  pointer-events: none;
  outline: none;
}
[type=range], [type=range]::-webkit-slider-thumb {
  -webkit-appearance: none;
}
[type=range].sliders::-webkit-slider-runnable-track {
  box-sizing: border-box;
  border: none;
  width: 100%;
  height: var(--sdds-slider-track-height);
  background: var(--sdds-slider-track-color);
  border-radius: 2px;
  background: linear-gradient(var(--sdds-slider-fill-color), var(--sdds-slider-fill-color)) 0/var(--sx) 100% no-repeat var(--sdds-slider-track-color);
  background: linear-gradient(to right, var(--sdds-slider-track-color) var(--minvalpx), var(--sdds-slider-fill-color) var(--minvalpx) var(--maxvalpx), var(--sdds-slider-track-color) var(--maxvalpx)) 100% no-repeat;
}
[type=range].sliders::-moz-range-track {
  box-sizing: border-box;
  border: none;
  width: 100%;
  height: var(--sdds-slider-track-height);
  background: var(--sdds-slider-track-color);
  border-radius: 2px;
  background: linear-gradient(var(--sdds-slider-fill-color), var(--sdds-slider-fill-color)) 0/var(--sx) 100% no-repeat var(--sdds-slider-track-color);
  background: linear-gradient(to right, var(--sdds-slider-track-color) var(--minvalpx), var(--sdds-slider-fill-color) var(--minvalpx) var(--maxvalpx), var(--sdds-slider-track-color) var(--maxvalpx)) 100% no-repeat;
}
[type=range].sliders::-ms-track {
  box-sizing: border-box;
  border: none;
  width: 100%;
  height: var(--sdds-slider-track-height);
  background: var(--sdds-slider-track-color);
  border-radius: 2px;
  background: linear-gradient(var(--sdds-slider-fill-color), var(--sdds-slider-fill-color)) 0/var(--sx) 100% no-repeat var(--sdds-slider-track-color);
  background: linear-gradient(to right, var(--sdds-slider-track-color) var(--minvalpx), var(--sdds-slider-fill-color) var(--minvalpx) var(--maxvalpx), var(--sdds-slider-track-color) var(--maxvalpx)) 100% no-repeat;
}
[type=range].sliders-right::-moz-range-track {
  background-color: transparent !important;
}
[type=range]::-webkit-slider-runnable-track {
  box-sizing: border-box;
  border: none;
  width: 100%;
  height: var(--sdds-slider-track-height);
  background: var(--sdds-slider-track-color);
  border-radius: 2px;
  background: linear-gradient(var(--sdds-slider-fill-color), var(--sdds-slider-fill-color)) 0/var(--sx) 100% no-repeat var(--sdds-slider-track-color);
}
[type=range]::-moz-range-track {
  box-sizing: border-box;
  border: none;
  width: 100%;
  height: var(--sdds-slider-track-height);
  background: var(--sdds-slider-track-color);
  border-radius: 2px;
  background: linear-gradient(var(--sdds-slider-fill-color), var(--sdds-slider-fill-color)) 0/var(--sx) 100% no-repeat var(--sdds-slider-track-color);
}
[type=range]::-ms-track {
  box-sizing: border-box;
  border: none;
  width: 100%;
  height: var(--sdds-slider-track-height);
  background: var(--sdds-slider-track-color);
  border-radius: 2px;
  background: linear-gradient(var(--sdds-slider-fill-color), var(--sdds-slider-fill-color)) 0/var(--sx) 100% no-repeat var(--sdds-slider-track-color);
}
[type=range]::-ms-fill-lower {
  height: var(--sdds-slider-track-height);
  background: var(--sdds-slider-fill-color);
}
[type=range]::-webkit-slider-thumb {
  margin-top: calc(0.5 * (
          var(--sdds-slider-track-height) - var(--sdds-slider-thumb-diameter)
        ));
  box-sizing: border-box;
  border: none;
  width: var(--sdds-slider-thumb-diameter);
  height: var(--sdds-slider-thumb-diameter);
  border-radius: 50%;
  background: var(--sdds-slider-fill-color);
  cursor: pointer;
  pointer-events: auto;
  position: relative;
  z-index: 10;
}
[type=range]:hover::-webkit-slider-thumb {
  margin-top: calc(0.5 * (var(--sdds-slider-track-height) - 21px));
  width: 21px;
  height: 21px;
  box-shadow: 0 0 1px 16px rgba(4, 30, 66, 0.08);
  cursor: pointer;
}
[type=range]:active::-webkit-slider-thumb {
  margin-top: calc(0.5 * (var(--sdds-slider-track-height) - 21px));
  width: 21px;
  height: 21px;
  box-shadow: 0 0 1px 16px rgba(4, 30, 66, 0.16);
  cursor: grabbing;
}
[type=range]::-moz-range-thumb {
  box-sizing: border-box;
  border: none;
  width: var(--sdds-slider-thumb-diameter);
  height: var(--sdds-slider-thumb-diameter);
  border-radius: 50%;
  background: var(--sdds-slider-fill-color);
  cursor: pointer;
  pointer-events: auto;
  position: relative;
  z-index: 10;
}
[type=range]:active::-moz-range-thumb, [type=range]:active::-ms-thumb {
  margin-top: calc(0.5 * (var(--sdds-slider-track-height) - 21px));
  width: 21px;
  height: 21px;
  box-shadow: 0 0 1px 16px rgba(4, 30, 66, 0.16);
  cursor: grabbing;
}
[type=range]::-ms-thumb {
  margin-top: 0;
  box-sizing: border-box;
  border: none;
  width: var(--sdds-slider-thumb-diameter);
  height: var(--sdds-slider-thumb-diameter);
  border-radius: 50%;
  background: var(--sdds-slider-fill-color);
  cursor: pointer;
  pointer-events: auto;
  position: relative;
  z-index: 10;
}
[type=range]::-ms-tooltip {
  display: none;
}

.input-text {
  width: 25px;
  height: 20px;
  text-align: center;
  font-size: 10px;
  margin-left: 10px;
  border: 0;
  border-radius: 10%;
  border-bottom: 1px solid var(--sdds-slider-track-color);
  background-color: var(--sdds-slider-track-color);
}
.input-text:hover, .input-text:focus {
  outline: none !important;
  border: 0;
  border-bottom: 1px solid var(--sdds-slider-fill-color);
}
.input-text.input-text-error {
  outline: none !important;
  border: 0;
  border-bottom: 1px solid var(--sdds-slider-error-red-color);
}

button {
  border: 0;
  width: 20px;
  height: 16px;
  background: transparent;
  padding: 0;
  cursor: pointer;
}
button.plus {
  background-image: url("data:image/svg+xml,%3Csvg width='16' height='16' viewBox='0 0 16 16' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath fill-rule='evenodd' clip-rule='evenodd' d='M8.49994 13.5104C8.49994 13.7865 8.27608 14.0104 7.99994 14.0104C7.7238 14.0104 7.49994 13.7865 7.49994 13.5104V8.50002H2.48964C2.2135 8.50002 1.98964 8.27617 1.98964 8.00002C1.98964 7.72388 2.2135 7.50002 2.48964 7.50002H7.49994V2.48968C7.49994 2.21353 7.7238 1.98968 7.99994 1.98968C8.27608 1.98968 8.49994 2.21353 8.49994 2.48968V7.50002H13.5103C13.7865 7.50002 14.0103 7.72388 14.0103 8.00002C14.0103 8.27617 13.7865 8.50002 13.5103 8.50002H8.49994V13.5104Z' fill='%230D0F13'/%3E%3C/svg%3E");
  margin-left: 16px;
}
button.minus {
  background-image: url("data:image/svg+xml,%3Csvg width='16' height='16' viewBox='0 0 16 16' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath fill-rule='evenodd' clip-rule='evenodd' d='M1.98965 8.00006C1.98965 8.2762 2.21351 8.50006 2.48965 8.50006H13.5104C13.7865 8.50006 14.0104 8.2762 14.0104 8.00006C14.0104 7.72392 13.7865 7.50006 13.5104 7.50006L2.48965 7.50006C2.21351 7.50006 1.98965 7.72392 1.98965 8.00006Z' fill='%230D0F13'/%3E%3C/svg%3E");
  margin-right: 16px;
}

.rangevalue {
  --range: calc(var(--max) - var(--min));
  --ratio: calc((var(--val) - var(--min)) / var(--range));
  --newPosition: calc(38px - (var(--ratio) * 76) * 1px);
  --leftspan: calc(var(--ratio) * 100%);
  background-color: var(--sdds-slider-span-color);
  color: var(--sdds-slider-track-color);
  position: absolute;
  top: -53px;
  left: calc(var(--leftspan) + var(--newPosition) + 0px);
}
.rangevalue span {
  padding: 8px;
  font-family: "Scania Sans Semi Condensed", "Scania Sans Condensed", Arial, Helvetica, sans-serif;
  font-size: 4rem;
  line-height: 6rem;
  letter-spacing: -0.01em;
  background-color: var(--sdds-slider-span-color);
  color: var(--sdds-white);
  text-align: center;
  display: block;
  position: absolute;
  left: 50%;
  transform: translate(-50%, 0);
  border-radius: 4px;
}
.rangevalue span::before {
  content: "";
  position: absolute;
  width: 8px;
  height: 8px;
  clip-path: polygon(0% 0%, 100% 100%, 0% 100%);
  border-radius: 0 0 0 1px;
  transform: rotate(-45deg);
  background-color: var(--sdds-slider-span-color);
  bottom: -3px;
  left: calc(50% - 4px);
}